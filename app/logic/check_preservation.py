def get_b_value(b_str):
   return int(b_str[1:])  # Lấy số sau chữ 'b'
...
def check_information_preservation(attributes, decomposition, dependencies):
  ...
  for X, Y in dependencies:
              ...
              if i != j and all(matrix[i][k] == matrix[j][k] for k in x_idx):
                  for y in y_idx:
                      if matrix[i][y] != matrix[j][y]:
                          # Lấy giá trị b nhỏ nhất
                          if matrix[i][y].startswith('b') and matrix[j][y].startswith('b'):
                              val = 'b' + str(min(get_b_value(matrix[i][y]), get_b_value(matrix[j][y])))
                          else:
      ...
      if changed:
          ...
          steps_matrices.append({
              'step': f"**Xét phụ thuộc hàm {''.join(X)}->{''.join(Y)}**: Không có thay đổi",
              'matrix': None
          })
